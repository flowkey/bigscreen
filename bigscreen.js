...
*/
BigScreen = (function(root, document, iframe) {
"use strict";
...
var callOnEnter = function(actualElement) {
    ...
    if (elements.length === 1) {
        bigscreen._runCallback('onenter', bigscreen.element);
    }
...
var callOnError = function(reason, element) {
    if (elements.length > 0) {
        ...
        bigscreen._runCallback('onerror', element, reason);
    }
};

var _callbacks = {
        onenter: new CallbackList(),
        onexit: new CallbackList(),
        onchange: new CallbackList(),
        onerror: new CallbackList()
};
...
var bigscreen = {
    ...
    _runCallback: function(name, element, error){
       _callbacks[name].run(element, error);
       bigscreen[name](element, error);
    },

    onEnter: _callbacks.onenter.add.bind(_callbacks.onenter),
    onExit: _callbacks.onexit.add.bind(_callbacks.onexit),
    onChange: _callbacks.onchange.add.bind(_callbacks.onchange),
    onError: _callbacks.onerror.add.bind(_callbacks.onerror),
...
try {
    ...
    if (fn.change) {
        document.addEventListener(fn.change, function onFullscreenChange(event) {
            bigscreen._runCallback('onchange', bigscreen.element);
            if (bigscreen.element) {
